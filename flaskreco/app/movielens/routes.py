...
def search_title():
    ...
    uri='http://' + app.config['ELASTIC_USR'] + ':' + app.config['ELASTIC_PWD'] + '@' + app.config['ELASTIC_HOST'] + ':9200/demo/movies/_search?pretty=true'
...
def index():
    ...
    if request.form.get('userId') != None:
        ...
        return render_template('movielens/search.html', movie=movieForm, movieTitle=title, movieVal=movieId, user=userForm, userId=userId, html=user_html, hr_html=x_html)
